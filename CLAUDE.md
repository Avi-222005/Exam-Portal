...
**Scorix** - open-source (AGPL-3.0) coding exam platform. NestJS 11 backend + Vue 3 frontend, served as a single container. Self-hosted Judge0 for code execution, ICPC-scored leaderboards, white-label branding via env vars.

- **Name:** Scorix (default in `client/src/config/brand.ts` and `server/src/config/env.ts`)
- **License:** AGPL-3.0
...
- `VITE_APP_NAME` / `APP_NAME` - app name (default: `Scorix`)
