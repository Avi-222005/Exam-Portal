...
import { LANGUAGE_MAP, LEGACY_LANGUAGE_FALLBACKS } from '../config/constants';
...
export class SubmissionsService {
  ...
  async submit(
    ...
    const effectiveLanguageId =
      LEGACY_LANGUAGE_FALLBACKS[languageId] ?? languageId;

    const languageName =
      Object.keys(LANGUAGE_MAP).find((k) =>
        LANGUAGE_MAP[k].includes(effectiveLanguageId),
      ) || 'unknown';

    // Enforce the exam's allowed languages (allow either original or normalized ID)
    const isAllowed = [languageId, effectiveLanguageId].some((id) =>
      exam.allowedLanguages.includes(id),
    );
    if (!isAllowed) {
