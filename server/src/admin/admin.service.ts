...
export class AdminService {
  ...
  async listExams(
  ...
  ): Promise<PaginatedResponse<Exam & { problems: unknown[] }>> {
    ...
    const data = exams.map((exam) => {
      const examMappings = mappings.filter((m) => m.examId === exam.id);
      const problems = examMappings.map((m) => ({
        ...m.problem,
  ...
  async getExamEnrollments(examId: number) {
  ...
  async batchEnroll(examId: number, dto: BatchEnrollDto) {
    await this.getExam(examId);
    const targetUsers = new Map<number, User>();

    if (dto.userIds?.length) {
      const users = await this.userRepo.find({ where: { id: In(dto.userIds) } });
      users.forEach((u) => targetUsers.set(u.id, u));
    }

    if (dto.rollNumbers?.length) {
      const cleanRolls = dto.rollNumbers.map((r) => r.trim()).filter(Boolean);
      if (cleanRolls.length) {
        const users = await this.userRepo.find({ where: { rollNumber: In(cleanRolls) } });
        users.forEach((u) => targetUsers.set(u.id, u));
      }
    }

    if (dto.emails?.length) {
      const cleanEmails = dto.emails.map((e) => e.trim().toLowerCase()).filter(Boolean);
      if (cleanEmails.length) {
        const users = await this.userRepo.find({ where: { email: In(cleanEmails) } });
        users.forEach((u) => targetUsers.set(u.id, u));
      }
    }

    const existing = await this.enrollmentRepo.find({ where: { examId } });
    const existingUserIds = new Set(existing.map((e) => e.userId));

    const toCreate: ExamEnrollment[] = [];
    ...
    for (const user of targetUsers.values()) {
      if (existingUserIds.has(user.id)) {
        ...
        toCreate.push(this.enrollmentRepo.create({ userId: user.id, examId }));
      }
    }
    ...
    return {
      totalFound: targetUsers.size,
      enrolledCount: toCreate.length,
      alreadyEnrolledCount,
    };
  }

  async removeEnrollment(examId: number, userId: number) {
    const res = await this.enrollmentRepo.delete({ examId, userId });
  ...
  async getProblem(id: number) {
    ...
    const examId = mappings.length > 0 ? mappings[0].examId : null;
    const examTitle = mappings.length > 0 ? mappings[0].exam?.title ?? null : null;
    const displayOrder = mappings.length > 0 ? mappings[0].displayOrder : 0;
    const exams = mappings.map((m) => ({
      id: m.examId,
      title: m.exam?.title ?? '',
      displayOrder: m.displayOrder,
    }));
  ...
  async updateProblem(id: number, dto: UpdateProblemDto) {
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      if (fields.examId !== undefined) {
        if (fields.examId && fields.examId > 0) {
          const exam = await manager.findOne(Exam, {
            where: { id: fields.examId },
          });
          if (!exam) throw new NotFoundException('Exam not found');

          const existingMapping = await manager.findOne(ProblemToExam, {
            where: { problemId: id },
          });

          if (existingMapping) {
            existingMapping.examId = fields.examId;
            if (fields.displayOrder !== undefined) {
              existingMapping.displayOrder = fields.displayOrder;
            }
            await manager.save(ProblemToExam, existingMapping);
          } else {
            let order = fields.displayOrder;
            if (order === undefined || order === null) {
              const maxMapping = await manager
                .createQueryBuilder(ProblemToExam, 'pte')
                .select('MAX(pte.displayOrder)', 'maxOrder')
                .where('pte.examId = :examId', { examId: fields.examId })
                .getRawOne<{ maxOrder: number | null }>();
              order = (maxMapping?.maxOrder ?? 0) + 1;
            }
            const newMapping = manager.create(ProblemToExam, {
              problemId: id,
            ...
            await manager.save(ProblemToExam, newMapping);
          }
        } else if (fields.examId === null || fields.examId === 0) {
          await manager.delete(ProblemToExam, { problemId: id });
        }
      } else if (fields.displayOrder !== undefined) {
  ...
  async deleteProblemsBulk(problemIds: number[]) {
    if (!problemIds || problemIds.length === 0) {
      return { deletedCount: 0 };
    }

    const uniqueIds = Array.from(new Set(problemIds));
    const result = await this.problemRepo.delete({ id: In(uniqueIds) });
    void this.scoringService.refreshLeaderboard();
    return { deletedCount: result.affected ?? uniqueIds.length };
  }
  ...
  async createProblemsBulk(
    ...
    try {
      let startOrder = 0;
      if (examId) {
        const maxMapping = await queryRunner.manager
          .createQueryBuilder(ProblemToExam, 'pte')
          .select('MAX(pte.displayOrder)', 'maxOrder')
          .where('pte.examId = :examId', { examId })
          .getRawOne<{ maxOrder: number | null }>();
        startOrder = (maxMapping?.maxOrder ?? -1) + 1;
      }
      ...
      for (let i = 0; i < dtos.length; i++) {
        const dto = dtos[i];
        if (dto.questionImageData) {
          this.assertImageSize(dto.questionImageData, `Question ${i + 1} image`);
        }

        const assignedOptions = this.validateAndAssignMcqOptions(dto);
        const { testCases, displayOrder, examId: _dtoExamId, ...problemData } =
          dto;
        const problemEntity = queryRunner.manager.create(Problem, {
          ...problemData,
          mcqOptions: assignedOptions,
        });
        const savedProblem = await queryRunner.manager.save(
          Problem,
          problemEntity,
        );

        const targetExamId = dto.examId || examId;
        if (targetExamId) {
          const order =
            displayOrder !== undefined && displayOrder !== null
              ? displayOrder
              : startOrder + i;
          const mapping = queryRunner.manager.create(ProblemToExam, {
            problemId: savedProblem.id,
            examId: targetExamId,
            displayOrder: order,
          });
          await queryRunner.manager.save(ProblemToExam, mapping);
        }

        if (testCases?.length) {
          const tcEntities = testCases.map((tc) =>
            queryRunner.manager.create(TestCase, {
              input: tc.input,
              expectedOutput: tc.expectedOutput,
              isVisible: tc.isVisible ?? false,
              displayOrder: tc.displayOrder,
          ...
          await queryRunner.manager.save(TestCase, tcEntities);
        }
      ...
      const ids = createdProblems.map((p) => p.id);
  ...
  async batchAssignProblemsToExam(examId: number, problemIds: number[]) {
    const exam = await this.examRepo.findOne({ where: { id: examId } });
    if (!exam) throw new NotFoundException('Exam not found');
    ...
    const maxMapping = await this.problemToExamRepo
      ...
      .where('pte.examId = :examId', { examId })
      .getRawOne<{ maxOrder: number | null }>();
    let nextOrder = (maxMapping?.maxOrder ?? -1) + 1;
    ...
    for (const problemId of problemIds) {
      ...
      if (!mapping) {
        const newMapping = this.problemToExamRepo.create({
          problemId,
          examId,
          displayOrder: nextOrder++,
        });
        await this.problemToExamRepo.save(newMapping);
        assigned++;
      }
    }
    return { assigned };
  ...
  async reorderExamProblems(
    examId: number,
    orders: { problemId: number; displayOrder: number }[],
  ) {
    for (const item of orders) {
      await this.problemToExamRepo.update(
        { examId, problemId: item.problemId },
  ...
  async listAllProblems(
    examId?: number,
    pagination?: PaginationDto,
  ): Promise<PaginatedResponse<Problem & { examId?: number | null; examTitle?: string | null; exams?: Array<{ id: number; title: string; displayOrder: number }> }>> {
    const page = pagination?.page ?? 1;
    ...
    if (examId) {
      const qb = this.problemToExamRepo
        ...
        .leftJoinAndSelect('pte.exam', 'exam')
    ...
    const mappings = problemIds.length
      ? await this.problemToExamRepo.find({
    ...
    const enrichedData = data.map((p) => {
      const pMappings = mappings.filter((m) => m.problemId === p.id);
      const exams = pMappings.map((m) => ({
        id: m.examId,
        title: m.exam?.title ?? '',
        displayOrder: m.displayOrder,
      }));
      const examId = exams.length > 0 ? exams[0].id : null;
      const examTitle =
        exams.length > 0 ? exams.map((e) => e.title).join(', ') : null;
      const displayOrder = pMappings.length > 0 ? pMappings[0].displayOrder : 0;
      return {
        ...p,
        examId,
        examTitle,
