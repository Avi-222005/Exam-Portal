...
export interface CandidateEnrollmentRow {
  id: number;
  userId: number;
  examId: number;
  enrolledAt: string;
...
export async function getExamEnrollments(
  examId: number,
): Promise<CandidateEnrollmentRow[]> {
  const { data } = await api.get<CandidateEnrollmentRow[]>(
    `/admin/exams/${examId}/enrollments`,
  );
  return data;
}

export async function batchEnrollCandidates(
  examId: number,
  payload: { userIds?: number[]; rollNumbers?: string[]; emails?: string[] },
): Promise<{ totalFound: number; enrolledCount: number; alreadyEnrolledCount: number }> {
  const { data } = await api.post(
    `/admin/exams/${examId}/enrollments/batch`,
    payload,
  );
  return data;
}

export async function removeCandidateEnrollment(
  examId: number,
  userId: number,
): Promise<{ removed: boolean }> {
  const { data } = await api.delete(
    `/admin/exams/${examId}/enrollments/${userId}`,
  );
  return data;
}
...
export async function createProblemsBulk(
...
export async function batchAssignProblems(
  examId: number,
  problemIds: number[],
): Promise<{ assigned: number }> {
  const { data } = await api.post<{ assigned: number }>(
    `/admin/exams/${examId}/problems/batch-assign`,
    { problemIds },
  );
  return data;
}

export async function reorderExamProblems(
  examId: number,
  orders: { problemId: number; displayOrder: number }[],
): Promise<{ success: boolean }> {
  const { data } = await api.put<{ success: boolean }>(
    `/admin/exams/${examId}/problems/reorder`,
    { orders },
  );
  return data;
}
...
export async function deleteProblemsBulk(
  problemIds: number[],
