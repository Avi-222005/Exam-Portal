...
onMounted(async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      form.value = {
        examId: p.examId || (p.exams && p.exams.length > 0 ? p.exams[0].id : ''),
        title: p.title ?? '',
...
async function save() {
  ...
  try {
    const payload: Record<string, unknown> = {
      ...
      examId: form.value.examId ? Number(form.value.examId) : null,
    };
...
<template>
  <div class="max-w-[900px]">
    ...
    <form v-else class="flex flex-col gap-5" @submit.prevent="save">
      <!-- Exam selector -->
      <div class="flex flex-col gap-1.5 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <div class="flex items-center justify-between">
          <label class="text-[13px] font-bold text-slate-800 dark:text-slate-200">
            Assigned Exam
          </label>
          <span v-if="form.examId" class="text-xs text-blue-600 dark:text-blue-400 font-semibold">
            Linked to: {{ examName }}
          </span>
        ...
        <select
          v-model="form.examId"
          class="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white focus:border-blue-500 outline-hidden transition-colors"
        >
          <option value="">None (Question Bank / Standalone)</option>
          <option v-for="e in exams" :key="e.id" :value="e.id">
            {{ e.title }} (Exam #{{ e.id }})
          </option>
        </select>
        <p class="text-[11px] text-slate-400">
          Selecting an exam links this question directly to that exam.
        </p>
      </div>
